        ...
        package dev.toxi.world.wrapper;

        import com.google.common.base.Preconditions;
        import dev.toxi.world.MyWorld;
        import dev.toxi.world.config.*;
        import dev.toxi.world.event.WorldCreateEvent;
        import dev.toxi.world.event.WorldLoadEvent;
        import dev.toxi.world.event.WorldUnloadEvent;
...
public class SystemWorld {
    ...
    public static boolean create(UUID uniqueID, WorldTemplate template) {
        ...
        new BukkitRunnable() {
            @Override
            public void run() {
                MyWorld.getInstance().getAdapter().create(event.getWorldCreator(), sw, () -> {
                    // Fix for #16
                    new BukkitRunnable() {
                        @Override
                        public void run() {
                    ...
                    }.runTask(MyWorld.getInstance());
                });
            }
        }.runTaskLater(MyWorld.getInstance(), 1);
    ...
    public void directUnload(World w) {
        if (!Bukkit.isPrimaryThread()) {
            Bukkit.getScheduler().runTask(MyWorld.getInstance(), () -> directUnload(w));
            return;
        }
    ...
    private void unloadLater(World w) {
        if (!Bukkit.isPrimaryThread()) {
            Bukkit.getScheduler().runTask(MyWorld.getInstance(), () -> unloadLater(w));
            return;
        }
        ...
        unloadLaterTask = Bukkit.getScheduler().runTaskLater(MyWorld.getInstance(), () -> {
    ...
    public void load(Player p) {
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(MyWorld.getInstance(), new Runnable() {
            public void run() {
